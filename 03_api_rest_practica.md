...
En esta práctica se completó lo siguiente:

### 1. Corrección del POST `/api/users` para la generación automática de IDs
- Se modificó [UsersController.java](file:///c:/Users/MSI/Desktop/PPW/Backend/SPRINGBOOT/fundamentos01/src/main/java/ec/edu/ups/icc/fundamentos01/users/controllers/UsersController.java) para que el campo `id` no sea enviado desde el cliente en `CreateUserDto`.
- Se implementó un generador seguro y atómico en memoria utilizando `AtomicLong currentId = new AtomicLong(1);`.
- En el endpoint `@PostMapping`, se asigna el ID secuencial único a la entidad antes de guardarla mediante `user.setId(currentId.getAndIncrement());`.
- Se corrigió el tipo de la variable `@PathVariable` de `int` a `Long` en todos los métodos de búsqueda y eliminación para que coincida con el tipo de la entidad `UserModel`.

### 2. Estructuración y réplica de arquitectura para el recurso Products
Se crearon todas las clases bajo la estructura modular `ec.edu.ups.icc.fundamentos01.products`:
- **Modelo:** [ProductModel.java](file:///c:/Users/MSI/Desktop/PPW/Backend/SPRINGBOOT/fundamentos01/src/main/java/ec/edu/ups/icc/fundamentos01/products/models/ProductModel.java)
- **DTOs:**
  - [CreateProductDto.java](file:///c:/Users/MSI/Desktop/PPW/Backend/SPRINGBOOT/fundamentos01/src/main/java/ec/edu/ups/icc/fundamentos01/products/dtos/CreateProductDto.java) (sin campos `id` ni `createdAt`)
  - [UpdateProductDto.java](file:///c:/Users/MSI/Desktop/PPW/Backend/SPRINGBOOT/fundamentos01/src/main/java/ec/edu/ups/icc/fundamentos01/products/dtos/UpdateProductDto.java)
  - [PartialUpdateProductDto.java](file:///c:/Users/MSI/Desktop/PPW/Backend/SPRINGBOOT/fundamentos01/src/main/java/ec/edu/ups/icc/fundamentos01/products/dtos/PartialUpdateProductDto.java)
  - [ProductResponseDto.java](file:///c:/Users/MSI/Desktop/PPW/Backend/SPRINGBOOT/fundamentos01/src/main/java/ec/edu/ups/icc/fundamentos01/products/dtos/ProductResponseDto.java)
- **Mapeador:** [ProductMapper.java](file:///c:/Users/MSI/Desktop/PPW/Backend/SPRINGBOOT/fundamentos01/src/main/java/ec/edu/ups/icc/fundamentos01/products/mappers/ProductMapper.java) para conversiones bidireccionales DTO ↔ Modelo.
- **Controlador:** [ProductsController.java](file:///c:/Users/MSI/Desktop/PPW/Backend/SPRINGBOOT/fundamentos01/src/main/java/ec/edu/ups/icc/fundamentos01/products/controllers/ProductsController.java) implementando los 6 endpoints CRUD en forma 100% funcional (Streams API, Lambdas y `removeIf`).

---

# 10. Resultados y evidencias

A continuación se adjuntan las capturas del consumo de los endpoints REST correspondientes al recurso `Products` desde Postman:

### 1. GET `/api/products` (Listado completo con 3 productos creados)
Muestra la lista de los 3 productos ("Laptop", "Mouse" y "Teclado Mecanico") registrados correctamente con sus IDs autogenerados secuencialmente.

![GET /api/products con 3 productos](assets/03_01practica.png)

---

### 2. GET `/api/products/:id` (Obtención de un producto existente)
Búsqueda de producto por su ID único (`1`).

![GET /api/products/:id con un producto existente](assets/03_02practica.png)

---

### 3. DELETE `/api/products/:id` (Eliminación de un producto existente)
Eliminación correcta del producto con ID `1`.

![DELETE /api/products/:id de un producto existente](assets/03_03practica.png)

---

### 4. DELETE `/api/products/:id` (Eliminación de un producto que no existe)
Respuesta de error al intentar eliminar un producto con ID inexistente (`999`).

![DELETE /api/products/:id de un producto inexistente](assets/03_04practica.png)